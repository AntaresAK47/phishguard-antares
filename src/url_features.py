...
def split_raw_links(value):
    ...
    # Acepta el token neutralizado [.] dentro del enlace (ejemplo:
    # hxxps://dominio[.]com), sin dejar de cortar en separadores.
    if not links:
...
def web_features(web_urls):
    ...
    for url in web_urls:
        ...
        # Las URL son entrada adversaria: una URL malformada no debe abortar
        # la extracción completa. Si urlparse falla (por ejemplo, corchetes
        # residuales interpretados como IPv6), se degrada a un análisis
        # puramente léxico de la cadena, sin host.
        try:
            ...
            host = ""
            scheme = "https" if u.lower().startswith("https://") else ""
        full_text = u.lower()
        ...
        uses_https = max(uses_https, int(scheme == "https"))
